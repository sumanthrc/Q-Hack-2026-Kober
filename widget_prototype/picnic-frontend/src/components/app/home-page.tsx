    ...
    <div className="absolute inset-0 flex flex-col bg-[#F8F8FA]">
      ...
      <div className="bg-white px-4 pt-2 pb-3 border-b border-gray-100">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-3.5 rounded-full bg-white border-none text-sm text-gray-900 outline-none shadow-sm focus:ring-2 focus:ring-[#E1002A]/20 transition-colors"
          />
        </div>
      </div>
      ...
      <div className="px-4 py-2 flex gap-2 overflow-x-auto no-scrollbar">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors shadow-sm ${
            !selectedCategory
              ? "bg-[#E1002A] text-white"
              : "bg-white text-[#6B7280] border border-[#F3F4F6] font-semibold hover:bg-gray-50"
          }`}
        >
          All
        </button>
        ...
        {loading ? (
          <div className="grid grid-cols-2 gap-2.5">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.03)] p-3 animate-pulse">
                <div className="h-12 w-12 mx-auto rounded-lg bg-gray-100 mb-3" />
            ...
            {products.map((product) => {
              ...
              return (
                <motion.div
                  key={product.id}
                  layout
                  className="bg-white p-3 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.03)] flex flex-col"
                >
                  <div className="text-3xl text-center py-3 bg-[#F9F9FB] rounded-xl">
                    {product.emoji}
                  </div>
                  <p className="font-bold text-sm leading-snug text-gray-900 truncate mt-2">
                    {product.name}
                  </p>
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    <span className="text-[#E1002A] font-extrabold text-lg">
                      €{product.price.toFixed(2)}
                    </span>
                    <motion.button
                      whileTap={{ scale: 0.9 }}
                      onClick={() => handleAdd(product.id)}
                      className={`w-7 h-7 rounded-xl p-1.5 flex items-center justify-center transition-colors shadow-sm ${
                        isJustAdded
